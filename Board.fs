...
module Board =
    ...
    let isValidAttack cell (board: Board) moveTo =
        if moveIsInBounds moveTo then
            ...
            | Cell { side = destSide }, Cell { side = side } -> destSide <> side
            | _ -> false
        else
            false
    ...
    let getValidAttacks f g movePacks =
        [| for movePack in movePacks do
               ...
               if Array.isEmpty validMoves |> not
                  && f validMoves.[0] then
                   yield validMoves.[0] |]
    ...
    let validateActualMove (board: Board) fromI toI =
        ...
        | Cell { side = side } ->
            let newBoard =
                { cells =
                      [| for i in 0 .. 63 ->
                             if i = fromI then EmptyCell
                             else if i = toI then board.cells.[fromI]
                             else board.cells.[i] |] }
            ...
            let attacks =
                ...
                |> getBoardAttacks
                    (function
                    | Cell { side = toSide }, _ -> side <> toSide
                    | _ -> false)
            ...
            attacks |> Array.contains kingLocation |> not
