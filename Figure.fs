...
module Figure =
    ...
    let private _getDirections fig =
        let getLines count =
           [|
               for x, y in List.zip [1; 0; -1; 0] [0; 1; 0; -1] ->
                   MoveDirection(count, (x, y))
           |]
    ...
    let getDirections fig =
        match fig with
        | King ->
            (FigMoves.KingMoves, FigMoves.KingMoves)
        | Queen ->
            (FigMoves.QueenMoves, FigMoves.QueenMoves)
        | Bishop ->
            (FigMoves.BishopMoves, FigMoves.BishopMoves)
        | Rook ->
            (FigMoves.RookMoves, FigMoves.RookMoves)
        | Knight ->
            (FigMoves.KnightMoves, FigMoves.KnightMoves)
